...
to quickly create a Cobra application.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if active == true {
				...
				for _, ts := range tss {
					if category != "" {
						if ts.Category == category {
							data = append(data, []string{ts.StartTime.Format("2006-01-02 3:4:5pm"), ts.EndTime.Sub(ts.StartTime).String(), ts.Project, ts.Category, ts.Comment})
				...
				table := tablewriter.NewWriter(os.Stdout)
				table.SetHeader([]string{"Date", "Duration", "Project", "Category", "Comment"})
				table.SetBorder(false)
